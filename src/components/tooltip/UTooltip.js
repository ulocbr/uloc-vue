...
import WindowChild from '../../mixins/window-parent.js'

export default {
  name: 'UTooltip',
  mixins: [ModelToggleMixin, CanRenderMixinMixin, WindowChild],
  props: {
    anchor: {
      type: String,
      default: 'top middle',
  ...
  methods: {
    __show () {
      ...
      if (this.$uloc.platform.is.mobile) {
  ...
  render (h) {
    ...
    return h('div', { staticClass: 'u-tooltip animate-popup' }, [
  ...
  mounted () {
    this.$nextTick(() => {
      ...
      */
      ...
      if (this.anchorEl.classList.contains('u-btn-inner')) {
