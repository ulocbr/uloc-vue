...
export default {
  ...
  computed: {
    ...
    computedAddIcon () {
      return this.addIcon || this.$uloc.icon.chipsInput.add
    },
  ...
  render (h) {
    return h(UInputFrame, {
      ...
      this.model.map((label, index) => {
        ...
        h('input', {
          ref: 'input',
          staticClass: 'col u-input-target',
      ...
      this.isLoading
        ...
        : ((this.editable && h(UIcon, {
          slot: 'after',
          staticClass: 'u-if-control',
          'class': { invisible: this.input.length === 0 },
          props: { name: this.computedAddIcon, type: 'material' },
          nativeOn: {
