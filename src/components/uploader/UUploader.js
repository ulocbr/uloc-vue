import UInputFrame from '../input-frame/UInputFrame.js'
...
export default {
  ...
  computed: {
    ...
    classes () {
      return {
        'u-uploader-expanded': this.expanded,
        'u-uploader-dark': this.dark,
        'u-uploader-files-no-border': this.isInverted || !this.hideUnderline
      }
    },
    progressColor () {
  ...
  methods: {
    ...
    __pick () {
      if (!this.addDisabled && this.$uloc.platform.is.mozilla) {
  ...
  render (h) {
    const child = [
      h('div', {
        staticClass: 'col u-input-target ellipsis',
    ...
    if (this.uploading) {
      child.push(
        h(USpinner, {
          slot: 'after',
          staticClass: 'u-if-end self-center',
          props: { size: '24px' }
        }),
        h(UIcon, {
          slot: 'after',
          staticClass: 'u-if-end self-center u-if-control',
          props: {
            name: this.$uloc.icon.uploader[`clear${this.isInverted ? 'Inverted' : ''}`]
          },
          nativeOn: {
            click: this.abort
          }
      ...
      child.push(
        h(UIcon, {
          slot: 'after',
          staticClass: 'u-uploader-pick-button self-center u-if-control relative-position overflow-hidden',
          props: {
            name: this.$uloc.icon.uploader.add
          },
          attrs: {
          ...
          h('input', {
            ref: 'file',
            staticClass: 'u-uploader-input absolute-full cursor-pointer',
            attrs: Object.assign({
              type: 'file',
      ...
      if (!this.hideUploadButton) {
        child.push(
          h(UIcon, {
            slot: 'after',
            staticClass: 'u-if-control self-center',
            props: {
              name: this.$uloc.icon.uploader.upload
            },
            attrs: {
    ...
    if (this.hasExpandedContent) {
      child.push(
        h(UIcon, {
          slot: 'after',
          staticClass: 'u-if-control generic_transition self-center',
          'class': { 'rotate-180': this.expanded },
          props: {
            name: this.$uloc.icon.uploader.expand
          },
          nativeOn: {
    ...
    return h('div', {
      staticClass: 'u-uploader relative-position',
      'class': this.classes,
      ...
      h(USlideTransition, [
        h('div', {
          ...
          h(UList, {
            staticClass: 'u-uploader-files u-py-none scroll',
            style: this.filesStyle,
            props: { dark: this.dark }
          }, this.files.map(file => {
            return h(UItem, {
              key: file.name + file.__timestamp,
              staticClass: 'u-uploader-file u-pa-xs'
            }, [
              (!this.hideUploadProgress && h(UProgress, {
                staticClass: 'u-uploader-progress-bg absolute-full',
                props: {
              ...
              (!this.hideUploadProgress && h('div', {
                staticClass: 'u-uploader-progress-text absolute'
              ...
              h(UItemSide, {
                props: file.__img
                  ...
                  : {
                    icon: this.$uloc.icon.uploader.file,
                    color: this.color
                  }
              }),
              ...
              h(UItemSide, { props: { right: true } }, [
                h(UItemTile, {
                  staticClass: 'cursor-pointer',
                  props: {
                    icon: this.$uloc.icon.uploader[file.__doneUploading ? 'done' : 'clear'],
                    color: this.color
                  },
                  nativeOn: {
      ...
      (this.dnd && h('div', {
        staticClass: 'u-uploader-dnd flex row items-center justify-center absolute-full',
        'class': this.dndClass,
        on: {
