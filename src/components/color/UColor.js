...
import UPopover from '../popover/UPopover'
...
export default {
  ...
  data () {
    let data = this.isPopover ? {} : {
      transition: process.env.THEME === 'ios' ? 'u-modal-bottom' : 'u-modal'
    }
    data.focused = false
  ...
  render (h) {
    return h(UInputFrame, {
      ...
      this.isClearable
        ? h(UIcon, {
          slot: 'after',
          props: { name: this.$uloc.icon.input[`clear${this.isInverted ? 'Inverted' : ''}`], type: 'material' },
      ...
      h(UIcon, {
        slot: 'after',
        props: { name: this.$uloc.icon.input.dropdown, type: 'material' },
        staticClass: 'u-if-control'
      })
    ])
  }
