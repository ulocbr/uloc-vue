...
import USlider from '../slider/USlider.js'
...
export default {
  ...
  computed: {
    ...
    saturationPointerStyle () {
      return {
        ...
        [this.$uloc.i18n.rtl ? 'right' : 'left']: `${this.model.s}%`
      }
    },
    inputsArray () {
      const inp = ['r', 'g', 'b']
  ...
  render (h) {
    return h('div', {
      staticClass: 'u-color',
      'class': { disabled: this.disable, 'u-color-dark': this.dark }
    }, [
  ...
  methods: {
    __getSaturation (h) {
      return h('div', {
        ref: 'saturation',
        staticClass: 'u-color-saturation non-selectable relative-position overflow-hidden cursor-pointer',
        ...
        h('div', { staticClass: 'u-color-saturation-white absolute-full' }),
        h('div', { staticClass: 'u-color-saturation-black absolute-full' }),
        h('div', {
          staticClass: 'absolute',
          style: this.saturationPointerStyle
        }, [
          this.model.hex !== void 0 ? h('div', { staticClass: 'u-color-saturation-circle' }) : null
        ])
      ])
    },
    __getSliders (h) {
      return h('div', {
        staticClass: 'u-color-sliders row items-center'
      }, [
        h('div', {
          staticClass: 'u-color-swatch m-t-sm m-l-md m-bsm non-selectable overflow-hidden'
        }, [
          h('div', { style: this.swatchColor, staticClass: 'fit' })
        ]),
        h('div', { staticClass: 'col u-pa-sm' }, [
          h('div', { staticClass: 'u-color-hue non-selectable' }, [
            h(USlider, {
              props: {
                value: this.model.h,
                color: 'white',
          ...
          this.hasAlpha
            ? h('div', { staticClass: 'u-color-alpha non-selectable' }, [
              h(USlider, {
                props: {
                  value: this.model.a,
                  color: 'white',
    ...
    __getNumericInputs (h) {
      return this.inputsArray.map(formatModel => {
        ...
        return h('div', { staticClass: 'col u-color-padding' }, [
          h('input', {
            attrs: {
              type: 'number',
              min: 0,
            ...
            staticClass: 'full-width text-center u-no-input-spinner',
            domProps: {
          ...
          h('div', { staticClass: 'u-color-label text-center uppercase' }, [
            formatModel
          ])
        ])
      })
    ...
    __getInputs (h) {
      const inputs = this.view === 'hex'
        ? [
          h('div', { staticClass: 'col' }, [
            ...
            h('div', { staticClass: 'u-color-label text-center' }, [
              `HEX${this.hasAlpha ? ' / A' : ''}`
            ])
          ])
        ]
      ...
      return h('div', {
        staticClass: 'u-color-inputs row items-center u-px-sm u-pb-sm'
      }, [
        h('div', { staticClass: 'col m-r-sm row no-wrap' }, inputs),
        h('div', [
          h(UBtn, {
            props: {
              flat: true,
