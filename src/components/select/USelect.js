import USearch from '../search/USearch.js'
...
import URadio from '../radio/URadio.js'
import UToggle from '../toggle/UToggle.js'
...
export default {
  ...
  render (h) {
    ...
    if (this.hasChips) {
      const el = h('div', {
        staticClass: 'col row items-center group u-input-chips',
    ...
    if (this.isClearable) {
      child.push(h(UIcon, {
        ...
        props: { name: this.$uloc.icon.input.clear },
        nativeOn: {
          click: this.clear
        }
      }))
    ...
    child.push(h(UIcon, {
      slot: 'after',
      staticClass: 'u-if-control',
      props: { name: this.$uloc.icon.input.dropdown }
    }))

    child.push(h(UPopover, {
      ref: 'popover',
      ...
      (this.filter && h(USearch, {
        ref: 'filter',
        staticClass: 'col-auto',
        style: 'padding: 10px;',
        props: {
          value: this.terms,
          placeholder: this.filterPlaceholder || this.$uloc.i18n.label.filter,
          debounce: 100,
          color: this.color,
          dark: this.dark,
          noParentField: true,
      ...
      (this.visibleOptions.length && h(UList, {
        ...
        return h(UItemWrapper, {
          key: index,
          'class': [
            opt.disable ? 'text-faded' : 'cursor-pointer',
            index === this.keyboardIndex ? 'u-select-highlight' : '',
