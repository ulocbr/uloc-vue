...
export default {
  ...
  computed: {
    ...
    controlBefore () {
      ...
      if (!this.noIcon) {
        before.unshift({
          icon: this.icon || this.$uloc.icon.search.icon,
          handler: this.focus
        })
      }
      return before
    ...
    controlAfter () {
      ...
      if (this.isClearable) {
        after.push({
          icon: this.$uloc.icon.search[`clear${this.isInverted ? 'Inverted' : ''}`],
          handler: this.clear
        })
      }
      return after
  ...
  render (h) {
    return h(UInput, {
      ...
      props: {
        ...
        placeholder: this.placeholder || this.$uloc.i18n.label.search,
