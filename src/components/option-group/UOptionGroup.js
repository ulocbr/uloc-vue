...
export default {
  ...
  created () {
    ...
    if (this.type === 'radio') {
      if (isArray) {
        console.error('u-option-group: model should not be array')
      }
    }
    else if (!isArray) {
      console.error('u-option-group: model should be array in your case')
    }
  },
  render (h) {
    return h(
      'div',
      {
        staticClass: 'u-option-group group',
        'class': { 'u-option-group-inline-opts': this.inline }
      },
      this.options.map(
        opt => h('div', [
          h(this.component, {
