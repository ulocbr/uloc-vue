...
export default {
  ...
  computed: {
    classes () {
      const cls = [
        `u-stepper-${this.vertical ? 'vertical' : 'horizontal'}`,
        `text-${this.color}`
      ]
      this.contractable && cls.push(`u-stepper-contractable`)
      return cls
    },
    hasSteps () {
  ...
  render (h) {
    return h('div', {
      staticClass: 'u-stepper column overflow-hidden relative-position',
      'class': this.classes
    }, [
      this.vertical
        ? null
        : h('div', {
          staticClass: 'u-stepper-header row items-stretch justify-between shadow-1',
