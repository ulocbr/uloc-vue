...
export default {
  ...
  methods: {
    __getDragging (event) {
      ...
      let
        percentage = getPercentage(event, dragging, this.$uloc.i18n.rtl),
        type
    ...
    __getHandle (h, lower, upper, edge, percentage, color, label) {
      return h('div', {
        ref: `handle${upper}`,
        staticClass: `u-slider-handle u-slider-handle-${lower}`,
        style: {
          [this.$uloc.i18n.rtl ? 'right' : 'left']: `${percentage * 100}%`,
        ...
        this.label || this.labelAlways
          ? h(UChip, {
            ...
            staticClass: 'u-slider-label no-pointer-events',
            'class': { 'label-always': this.labelAlways }
          }, [ label ])
          : null,
        process.env.THEME !== 'ios'
          ? h('div', { staticClass: 'u-slider-ring' })
          : null
      ])
    },
    __getContent (h) {
      return [
        h('div', {
          staticClass: 'u-slider-track active-track',
          style: {
            [this.$uloc.i18n.rtl ? 'right' : 'left']: `${this.percentageMin * 100}%`,
