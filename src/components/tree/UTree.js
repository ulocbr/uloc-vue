...
export default {
  ...
  computed: {
    ...
    computedIcon () {
      return this.icon || this.$uloc.icon.tree.icon
    },
  ...
  methods: {
    ...
    __getNodeMedia (h, node) {
      if (node.icon) {
        return h(UIcon, {
          staticClass: `u-tree-icon m-r-sm`,
          props: { name: node.icon }
        })
      }
      if (node.img || node.avatar) {
        return h('img', {
          staticClass: `u-tree-img m-r-sm`,
    ...
    __getNode (h, node) {
      ...
      if (body) {
        body = h('div', { staticClass: 'u-tree-node-body relative-position' }, [
      ...
      return h('div', {
        key,
        staticClass: 'u-tree-node',
        'class': { 'u-tree-node-parent': isParent, 'u-tree-node-child': !isParent }
      }, [
        h('div', {
          staticClass: 'u-tree-node-header relative-position row no-wrap items-center',
          'class': {
            'u-tree-node-link': meta.link,
            'u-tree-node-selected': meta.selected,
          ...
          meta.lazy === 'loading'
            ? h(USpinner, {
              staticClass: 'u-tree-node-header-media m-r-xs',
              props: { color: this.computedControlColor }
            })
            : (
              isParent
                ? h(UIcon, {
                  staticClass: 'u-tree-arrow m-r-xs transition-generic',
                  'class': { 'u-tree-arrow-rotate': meta.expanded },
        ...
        isParent
          ? h(USlideTransition, [
            h('div', {
              ...
              staticClass: 'u-tree-node-collapsible',
              'class': `text-${this.color}`
            }, [
              body,
              h('div', {
                staticClass: 'u-tree-children',
  ...
  render (h) {
    ...
    return h(
      'div', {
        staticClass: 'u-tree relative-position',
        'class': this.classes
      },
      children.length === 0
        ? (
          this.filter
            ? this.noResultsLabel || this.$uloc.i18n.tree.noResults
            : this.noNodesLabel || this.$uloc.i18n.tree.noNodes
        )
        : children
    )
  },
