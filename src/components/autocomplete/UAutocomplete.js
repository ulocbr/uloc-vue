...
export default {
  ...
  inject: {
    __input: {
      default () {
        console.error('UAutocomplete needs to be child of UInput, UChipsInput or USearch')
      }
    },
  ...
  render (h) {
    ...
    return h(UPopover, {
      ...
      this.computedResults.map((result, index) => h(UItemWrapper, {
        key: result.id || index,
        'class': {
          'u-select-highlight': this.keyboardIndex === index,
