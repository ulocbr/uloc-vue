...
export default {
  ...
  computed: {
    rtlDir () {
      return this.$uloc.i18n.rtl ? -1 : 1
    },
    arrowIcon () {
      const ico = [ this.$uloc.icon.carousel.left, this.$uloc.icon.carousel.right ]
      return this.$uloc.i18n.rtl
        ? ico.reverse()
        : ico
    },
    trackPosition () {
    ...
    computedUuickNavIcon () {
      return this.quickNavIcon || this.$uloc.icon.carousel.quickNav
    },
    computedStyle () {
    ...
    computedThumbnailIcon () {
      return this.thumbnailsIcon || this.$uloc.icon.carousel.thumbnails
    }
  },
  methods: {
    previous () {
    ...
    __getUuickNav (h) {
      ...
      if (slot) {
        ...
        for (let i = 0; i < this.slidesNumber; i++) {
          items.push(h(UBtn, {
            ...
            props: {
              icon: this.computedUuickNavIcon,
              round: true,
              flat: true,
              dense: true,
              color: this.color
      ...
      return h('div', {
        staticClass: 'u-carousel-quick-nav scroll text-center',
    ...
    __getThumbnails (h) {
      ...
      const nodes = [
        h(UBtn, {
          staticClass: 'u-carousel-thumbnail-btn absolute',
          props: {
        ...
        h('div', {
          staticClass: 'u-carousel-thumbnails scroll absolute-bottom',
  ...
  render (h) {
    return h('div', {
      staticClass: 'u-carousel',
      style: this.computedStyle,
      'class': { fullscreen: this.inFullscreen }
    }, [
      h('div', {
        staticClass: 'u-carousel-inner',
        ...
        h('div', {
          ...
          this.infiniteRight ? h('div', { staticClass: 'u-carousel-slide', style: `flex: 0 0 ${100}%` }) : null,
          this.$slots.default,
          this.infiniteLeft ? h('div', { staticClass: 'u-carousel-slide', style: `flex: 0 0 ${100}%` }) : null
        ])
      ]),
      this.arrows ? h(UBtn, {
        staticClass: 'u-carousel-left-arrow absolute',
        props: { color: this.color, icon: this.arrowIcon[0], fabMini: true, flat: true },
        directives: [{ name: 'show', value: this.canGoToPrevious }],
        on: { click: this.previous }
      }) : null,
      this.arrows ? h(UBtn, {
        staticClass: 'u-carousel-right-arrow absolute',
        props: { color: this.color, icon: this.arrowIcon[1], fabMini: true, flat: true },
        directives: [{ name: 'show', value: this.canGoToNext }],
        on: { click: this.next }
      }) : null,
      this.__getUuickNav(h),
