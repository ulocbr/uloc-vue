...
export default {
  ...
  data () {
    return this.hasObserver
      ? {}
      : { url: this.$uloc.platform.is.ie ? null : 'about:blank' }
  },
  methods: {
    onResize () {
      this.timer = null
  ...
  beforeCreate () {
    ...
    if (!this.hasObserver) {
      this.style = `${this.$uloc.platform.is.ie ? 'visibility:hidden;' : ''}display:block;position:absolute;top:0;left:0;right:0;bottom:0;height:100%;width:100%;overflow:hidden;pointer-events:none;z-index:-1;`
    }
  },
  mounted () {
    if (this.hasObserver) {
