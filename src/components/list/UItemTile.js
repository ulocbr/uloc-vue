...
export default {
  ...
  computed: {
    ...
    classes () {
      ...
      this.type && cls.push(`u-item-${this.type}`)

      if (this.inverted && (this.icon || this.letter)) {
        cls.push('u-item-inverted')
        cls.push('flex')
    ...
    if (this.icon) {
      if (this.inverted) {
        return h(this.tag, data, [
          h(UIcon, { props: { name: this.icon } }, this.$slots.default)
        ])
      }
      data.props = { name: this.icon }
    }

    return h(this.icon ? UIcon : this.tag, data, this.$slots.default)
  }
}
