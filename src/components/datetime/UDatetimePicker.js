...
export default {
  ...
  computed: {
    classes () {
      ...
      this.dark && cls.push('u-datetime-dark')
      this.minimal && cls.push('u-datetime-minimal')
    ...
    dateArrow () {
      const val = [ this.$uloc.icon.datetime.arrowLeft, this.$uloc.icon.datetime.arrowRight ]
      return this.$uloc.i18n.rtl ? val.reverse() : val
    },
    computedFormat24h () {
      return this.format24h !== 0
        ? this.format24h
        : this.$uloc.i18n.date.format24h
    },
    computedFirstDayOfWeek () {
      return this.firstDayOfWeek !== void 0
        ? this.firstDayOfWeek
        : this.$uloc.i18n.date.firstDayOfWeek
    },
    headerDayNames () {
      const
        days = this.$uloc.i18n.date.daysShort,
    ...
    monthString () {
      return `${this.$uloc.i18n.date.monthsShort[this.month - 1]}`
    },
    monthStamp () {
      return `${this.$uloc.i18n.date.months[this.month - 1]} ${this.year}`
    },
    weekDayString () {
      return this.$uloc.i18n.date.days[this.model.getDay()]
    },
  ...
  methods: {
    ...
    __getTopSection (h) {
      ...
      if (this.typeHasDate) {
        const content = [
          h('div', { staticClass: 'u-datetime-weekdaystring col-12' }, [
            this.weekDayString
          ]),

          h('div', { staticClass: 'u-datetime-datestring row flex-center' }, [
            h('span', {
              staticClass: 'u-datetime-link small col-auto col-md-12',
            ...
            h('span', {
              staticClass: 'u-datetime-link col-auto col-md-12',
      ...
      if (this.typeHasTime) {
        const content = [
          h('span', {
            staticClass: 'u-datetime-link col-md text-right u-pr-sm',
          ...
          h('span', {
            staticClass: 'u-datetime-link col-md text-left u-pl-sm',
        ...
        child.push(h('div', {
          staticClass: 'u-datetime-time row flex-center'
        }, [
          h('div', {
            staticClass: 'u-datetime-clockstring col-auto col-md-12 row no-wrap flex-center'
          }, content),

          (!this.computedFormat24h && h('div', {
            staticClass: 'u-datetime-ampm column col-auto col-md-12 justify-around'
          }, [
            h('div', {
              staticClass: 'u-datetime-link',
      ...
      return h('div', {
        staticClass: 'u-datetime-header column col-xs-12 col-md-4 justify-center'
      }, child)
    },
    ...
    __getYearView (h) {
      ...
      for (let i = this.yearMin; i <= max; i++) {
        content.push(h(UBtn, {
          key: `yi${i}`,
          staticClass: 'u-datetime-btn full-width',
      ...
      return h('div', {
        staticClass: `u-datetime-view-year full-width full-height`
      }, content)
    },
    ...
    __getMonthView (h) {
      ...
      for (let i = this.monthMin; i <= this.monthInterval; i++) {
        ...
        }, [ this.$uloc.i18n.date.months[i] ]))
      }

      return h('div', {
        staticClass: `u-datetime-view-month full-width full-height`
      }, content)
    },
    ...
    __getDayView (h) {
      ...
      for (let i = 1; i <= this.fillerDays; i++) {
        days.push(h('div', {
          key: `fd${i}`,
          staticClass: 'u-datetime-fillerday'
        }))
      }
      ...
      for (let i = min; i <= max; i++) {
        days.push(h('div', {
          ...
          'class': [this.color && i === this.day ? `text-${this.color}` : null, {
            'u-datetime-day-active': this.isValid && i === this.day,
            'u-datetime-day-today': i === this.today,
      ...
      return h('div', { staticClass: 'u-datetime-view-day' }, [
        h('div', { staticClass: 'row items-center content-center' }, [
          h(UBtn, {
            staticClass: 'u-datetime-arrow',
            attrs: { tabindex: -1 },
            props: {
              round: true,
              dense: true,
          ...
          h('div', { staticClass: 'col u-datetime-month-stamp' }, [
            this.monthStamp
          ]),
        ...
        h('div', {
          staticClass: 'u-datetime-weekdays row items-center justify-start'
        }, this.headerDayNames.map(day => h('div', { key: `dh${day}` }, [ day ]))),

        h('div', {
          staticClass: 'u-datetime-days row wrap items-center justify-start content-center'
        }, days)
      ])
    },
    ...
    __getClockView (h) {
      ...
      if (this.view === 'hour') {
        ...
        for (let i = init; i < max; i++) {
          content.push(h('div', {
            key: `hi${i}`,
            staticClass: `u-datetime-clock-position${cls}`,
            'class': [`u-datetime-clock-pos-${i}`, i === this.hour ? 'active' : '']
          }, [ h('span', [ i ]) ]))
        }
      }
      else {
        for (let i = 0; i < 12; i++) {
          ...
          content.push(h('div', {
            key: `mi${i}`,
            staticClass: 'u-datetime-clock-position',
            'class': ['u-datetime-clock-pos-' + i, five === this.minute ? 'active' : '']
          }, [
            h('span', [ five ])
          ]))
        }
      ...
      return h('div', {
        ...
        h('div', {
          staticClass: 'u-datetime-clock cursor-pointer',
          on: {
          ...
          h('div', { staticClass: 'u-datetime-clock-circle full-width full-height' }, [
            h('div', { staticClass: 'u-datetime-clock-center' }),
            h('div', {
              staticClass: 'u-datetime-clock-pointer',
  ...
  render (h) {
    ...
    return h('div', {
      staticClass: 'u-datetime row',
      'class': this.classes
    }, [
      (!this.minimal && this.__getTopSection(h)) || void 0,

      h('div', {
        staticClass: 'u-datetime-content col-xs-12 column',
        'class': this.contentClasses
      }, [
        h('div', {
          ref: 'selector',
          staticClass: 'u-datetime-selector auto row flex-center'
        }, [
