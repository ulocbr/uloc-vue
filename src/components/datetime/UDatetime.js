...
export default {
  ...
  created () {
    ...
    if (!this.isPopover) {
      this.transition = process.env.THEME === 'ios' ? 'u-modal-bottom' : 'u-modal'
    }
  },
  computed: {
    computedFormat () {
    ...
    actualValue () {
      ...
      return formatDate(this.value, this.computedFormat, /* for reactiveness */ this.$uloc.i18n.date)
    },
    computedValue () {
            ...
            ? h('div', {
              ...
              h(UBtn, {
                props: {
                  ...
                  flat: true,
                  label: this.cancelLabel || this.$uloc.i18n.label.cancel,
                  noRipple: true
                },
                on: {
                  click: () => {
              ...
              this.editable
                ? h(UBtn, {
                  props: {
                    color: this.modalBtnColor,
                    flat: true,
                    label: this.okLabel || this.$uloc.i18n.label.set,
                    noRipple: true,
                    disable: !this.model
                  },
                  on: {
  ...
  render (h) {
    return h(UInputFrame, {
      ...
      this.isClearable
        ? h(UIcon, {
          slot: 'after',
          props: { name: this.$uloc.icon.input[`clear${this.isInverted ? 'Inverted' : ''}`] },
