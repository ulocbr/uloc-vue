...
export default {
  ...
  computed: {
    ...
    buttonDef () {
      const
        e = this.$uloc.i18n.editor,
        i = this.$uloc.icon.editor
      ...
      return {
        ...
        h1: {cmd: 'formatBlock', param: 'H1', icon: i.header, tip: e.header1, htmlTip: `<h1 class="u-ma-none">${e.header1}</h1>`},
        h2: {cmd: 'formatBlock', param: 'H2', icon: i.header, tip: e.header2, htmlTip: `<h2 class="u-ma-none">${e.header2}</h2>`},
        h3: {cmd: 'formatBlock', param: 'H3', icon: i.header, tip: e.header3, htmlTip: `<h3 class="u-ma-none">${e.header3}</h3>`},
        h4: {cmd: 'formatBlock', param: 'H4', icon: i.header, tip: e.header4, htmlTip: `<h4 class="u-ma-none">${e.header4}</h4>`},
        h5: {cmd: 'formatBlock', param: 'H5', icon: i.header, tip: e.header5, htmlTip: `<h5 class="u-ma-none">${e.header5}</h5>`},
        h6: {cmd: 'formatBlock', param: 'H6', icon: i.header, tip: e.header6, htmlTip: `<h6 class="u-ma-none">${e.header6}</h6>`},
    ...
    buttons () {
      ...
      const def = this.definitions || this.fonts
        ? extend(
          ...
          getFonts(
            this.defaultFont,
            this.$uloc.i18n.editor.defaultFont,
            this.$uloc.icon.editor.font,
            this.fonts
          )
        )
        : this.buttonDef
  ...
  render (h) {
    ...
    if (this.hasToolbar) {
      const toolbarConfig = {
        staticClass: `u-editor-toolbar row no-wrap scroll-x`,
        'class': [
          { 'u-editor-toolbar-separator': !this.toolbarOutline && !this.toolbarPush },
          this.toolbarBackgroundClass
        ]
      }
      toolbars = []
      toolbars.push(h('div', extend({key: 'qedt_top'}, toolbarConfig), [
        h('div', { staticClass: 'row no-wrap u-editor-toolbar-padding fit items-center' }, getToolbar(h, this))
      ]))
      if (this.editLinkUrl !== null) {
        toolbars.push(h('div', extend({key: 'qedt_btm'}, toolbarConfig), [
          h('div', { staticClass: 'row no-wrap u-editor-toolbar-padding fit items-center' }, getLinkEditor(h, this))
        ]))
      }
