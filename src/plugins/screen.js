...
export default {
  ...
  install ({ $uloc, queues, Vue }) {
    ...
    const start = () => {
      ...
      if (style.getPropertyValue('--u-size-sm')) {
        ['sm', 'md', 'lg', 'xl'].forEach(name => {
          this.sizes[name] = parseInt(style.getPropertyValue(`--u-size-${name}`), 10)
        })
      }
    ...
    Vue.util.defineReactive($uloc, 'screen', this)
