...
function init ({ Vue }) {
  ...
  this.__vm = new Vue({
    ...
    render (h) {
      return h('div', { staticClass: 'u-notifications' }, positionList.map(pos => {
        const
        ...
        return h('transition-group', {
          key: pos,
          staticClass: `u-notification-list u-notification-list-${vert} fixed column ${classes}`,
          tag: 'div',
          props: {
            name: `u-notification-${pos}`,
            mode: 'out-in'
          }
...
export default {
  ...
  install (args) {
    ...
    args.$uloc.notify = this.create.bind(this)
    args.$uloc.notify.setDefaults = this.setDefaults
  }
}
